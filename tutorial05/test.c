...
/* 
 * The macro EXPECT_EQ_SIZE_T uses "%Iu" for MSVC because it is the format specifier for size_t in MSVC.
 * For other compilers, "%zu" is used as it is the standard format specifier for size_t in C99.
 */
#if defined(_MSC_VER)
...
static void test_parse_array() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        for (j = 0; j < i; j++) {
            ...
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(a, j)));
        }
    }
    lept_free(&v);
}
